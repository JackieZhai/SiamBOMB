...
from ..data.bounding_box_utils import masks_to_bboxes
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None):
        ...
        self.results_dir = '{}/{}/{}'.format('tracking_results', self.name, self.parameter_name)
        ...
        if os.path.isdir(tracker_module_abspath):
            tracker_module = importlib.import_module('..tracker.{}'.format(self.name))
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        output['image_shape'] = image.shaoe[:2]
    ...
    def run_video(self, videofilepath, optional_box=None, debug=None, visdom_info=None, save_results=False):
        ...
        def _build_init_info_op(boxes):
            info = {}
            info['init_bbox'] = OrderedDict()
            info['init_object_ids'] = []
            info['object_ids'] = []
            info['sequence_object_ids'] = []
            for i, box in enumerate(boxes):
                info['init_bbox'][i+1] = list(box)
                info['init_object_ids'].append(i+1)
                info['object_ids'].append(i+1)
                info['sequence_object_ids'].append(i+1)
            return info
        ...
        if optional_box is not None:
            ...
            out = tracker.initialize(frame, _build_init_info_op(optional_box))
            ...
            if 'target_bbox' in out:
                for obj_id, state in out['target_bbox'].items():
                    ...
                    cv.rectangle(frame_disp, (state[0], state[1]), (state[2] + state[0], state[3] + state[1]),
                                 (0, 255, 0), 2)
                    cv.putText(frame_disp, str(obj_id), (state[0], state[1]-5), cv.FONT_HERSHEY_SIMPLEX, 1,
                               (0, 255, 0), 2)
    ...
    def get_parameters(self):
        """Get parameters."""
        param_module = importlib.import_module('..parameter.{}.{}'.format(self.name, self.parameter_name))
