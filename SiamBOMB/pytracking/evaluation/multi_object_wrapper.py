...
class MultiObjectWrapper:
    ...
    def create_tracker(self):
        ...
        if self.fast_load:
            try:
                tracker = deepcopy(self.tracker_copy)
            except:
                pass
        if tracker is None:
    ...
    def _split_info(self, info):
        ...
        for obj_id in info['init_object_ids']:
            ...
            if 'init_bbox' in info and obj_id in info['init_bbox']:
                info_split[obj_id]['init_bbox'] = info['init_bbox'][obj_id]
                init_other[obj_id]['init_bbox'] = info['init_bbox'][obj_id]
            if 'init_mask' in info and obj_id in info['init_mask']:
    ...
    def martin_merge(self, out_all):
        ...
        if 'segmentation' in out_types and out_first['segmentation'] is not None:
            # Stack all masks
            segmentation_maps = [out['segmentation'] for out in out_all.values()]
            segmentation_list = []
    ...
    def merge_outputs(self, out_all):
        if hasattr(self.base_tracker_class, 'merge_results'):
            ...
            out_merged = self.martin_merge(out_all)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if info.get('sequence_object_ids', False) and len(info['sequence_object_ids']) > 0:
            ...
            temp_info['init_object_ids'] = info['sequence_object_ids']
            temp_info['sequence_object_ids'] = []
            sequence_info_split = self._split_info(temp_info)
            # for obj_init_info in sequence_info_split.values():
            #     obj_init_info['previous_output'] = prev_output
            # info['init_other'] = list(sequence_info_split.values())[0]['init_other']
        ...
        for obj_id in temp_initialized_ids:
            ...
            if info.get('sequence_object_ids', False) and \
                ...
                if obj_id in info['init_bbox']:  # Modify
                    ...
                    out = self.trackers[obj_id].initialize(image, sequence_info_split[obj_id])
                    if out is None:
                        out = {}
                    default = {'target_bbox': new_bbox,
                               'time': time.time() - start_time,
                               'segmentation': None}
                else:  # Delete
